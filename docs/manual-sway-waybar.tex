...
\title[Gestor de Ventanas en Mosaico (Tiling) y Barra de Estado para Wayland]{Guía Completa de Uso y Manejo de Sway y Waybar}
...
\begin{notabox}[Filosofía Wayland]
  A diferencia de X11, Wayland maneja cada ventana en su propio buffer aislado. Sway actúa como compositor, lo que implica mayor seguridad, menor latencia y soporte nativo de HiDPI sin configuraciones adicionales.
\end{notabox}
...
\begin{controlbox}[Autoevaluación]
  Si tienes Emacs en el Workspace 2 y quieres moverlo al Workspace 3 sin cambiar de escritorio, ¿qué combinación usas?
\end{controlbox}
...
\begin{notabox}[Scratchpad como Terminal Rápida]
  Configura el scratchpad con un tamaño reducido y la opción \texttt{floating enable} en Sway para tener una calculadora, monitor de sistema o terminal rápida siempre a un atajo de distancia.
\end{notabox}
...
\begin{warningbox}[Advertencia de Hardware]
  La tecla \textbf{Bloq Mayús} como \textbf{Escape} requiere configuración por hardware (firmware del teclado o utilidad like \texttt{setkeycodes}). Si no funciona, verifica que \texttt{interception-tools} o \texttt{kmonad} estén activos en tu sesión.
\end{warningbox}
...
\toprule
\textbf{Módulo} & \textbf{Información Mostrada} \\
\midrule
\texttt{sway/workspaces} & Espacios de trabajo activos con nombres personalizados \\
\texttt{sway/mode} & Estado actual de Sway (modo resize, etc.) \\
\texttt{clock} & Fecha y hora con formato \texttt{\%a \%d \%b | \%H:\%M} \\
\texttt{pulseaudio} & Volumen de audio con icono y estado mute \\
\texttt{backlight} & Brillo de pantalla en porcentaje \\
\texttt{battery} & Nivel de batería con estados warning (30\%) y critical (15\%) \\
\texttt{tray} & Bandeja del sistema (iconos de aplicaciones en segundo plano) \\
\bottomrule
\end{tabularx}
\end{center}
