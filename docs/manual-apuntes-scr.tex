...
\title[Guía Definitiva de Arquitectura Tipográfica, Estructura Bourbaki / IHÉS y Redacción Científica]{Manual de Referencia Oficial e Integral de la Clase \texttt{apuntes-scr.cls}}
...
\begin{notabox}[Objetivo Principal]
  Esta clase busca reproducir la estética editorial de los grandes tratados matemáticos (Bourbaki, EGA, SGA) manteniendo la legibilidad moderna y la compatibilidad con herramientas de edición asistida por computadora.
\end{notabox}
...
\begin{egabox}[Filosofía Bourbaki]
...
\begin{warningbox}[Advertencia Importante]
  La opción \texttt{fast} desactiva \texttt{minted} y las figuras vectoriales pesadas. Úsala únicamente para borradores de desarrollo. No la uses en documentos finales.
\end{warningbox}
...
\section{Entornos Complementarios de Razonamiento}

\subsection{Afirmaciones y Pruebas Locales (\texttt{claim} / \texttt{claimproof})}
Para lemas auxiliares dentro de una demostración mayor:

\begin{claim}[Restricción inyectiva]\label{clm:inyectividad}
...
\subsection{Casos y Condiciones}
Los entornos \texttt{caso} y \texttt{condicion} permiten estructurar demostraciones por análisis de casos con numeración automática:

\begin{conditions}[Continuidad]
    \conditem $f$ es continua en $x_0$.
    \conditem $g$ es derivable en $x_0$.
    \conditem $\lim_{x \to x_0} g(x) \neq 0$.
\end{conditions}

\subsection{Objetivos, Escolios y Recordatorios}
\begin{itemize}[leftmargin=8mm]
    \item \textbf{\texttt{objetive}}: Marca el objetivo de una sección o demostración.
    \item \textbf{\texttt{scholium} / \texttt{scholie}}: Notas marginales al estilo Bourbaki sobre implicaciones más profundas.
    \item \textbf{\texttt{rappel}}: Recordatorio de un resultado previo que se usará en la demostración.
\end{itemize}

\subsection{Convenciones}
El entorno \texttt{convenciones} marca supuestos globales del texto:

\begin{convencionbox}[Notación]
  Todos los anillos considerados son conmutativos con identidad salvo mención expresa.
\end{convencionbox}
...
\subsection{Ejercicios Estilo Bourbaki (\texttt{exercices})}
El entorno \texttt{exercices} genera una sección formal con enumeración alfabética en itálica:

\begin{exercices}[Ejercicios del Capítulo]
    \exer Calcular $\Spec(\Z)$ y describir sus puntos cerrados.
    \exerpro Probar que todo dominio PID es un dominio de ideales principales.
    \exerdif Sea $A$ un anillo noetheriano. Probar que $A[x]$ es noetheriano.
\end{exercices}

\subsection{Notas Históricas (\texttt{notehistorique})}
El entorno \texttt{notehistorique} permite insertar contextos históricos con estilo editorial Bourbaki:

\begin{notehistorique}[Origen de la noción de esquema]
...
\section{Optimización con \texttt{optidef}}
La clase integra el paquete \texttt{optidef} para escribir problemas de optimización de forma semántica:

\begin{center}
\small
\begin{tabularx}{\linewidth}{l X}
\toprule
\textbf{Macro} & \textbf{Uso} \\
\midrule
\texttt{\string\optmin} & minimizar \\
\texttt{\string\optmax} & maximizar \\
\texttt{\string\optst} & sujeto a \\
\bottomrule
\end{tabularx}
\end{center}
...
\section{Diagramas de Transición entre Cartas}
La macro \texttt{\string\transitiondiagram} genera diagramas de transición de variedades con dos cartas superpuestas:

\begin{center}
\transitiondiagram{U}{V}{W}{\phi}{\psi}{\varphi}{\theta}
\end{center}

\section{Mapas y Morfismos Estructurados}
Para describir aplicaciones entre espacios de forma tipográfica elegante:

\begin{itemize}[leftmargin=8mm]
    \item \texttt{\string\map\{f\}\{X\}\{Y\}\{x\}\{f(x)\}\{&\}}: genera el mapa $f: X \to Y$ con $x \mapsto f(x)$.
    \item \texttt{\string\inmap\{f\}\{X\}\{Y\}}: generación inline $f: X \to Y$.
    \item \texttt{\string\Fobj\{F\}\{X\}\{f\}\{Y\}\{F(x)\}}: describe el objeto $F = \{ f: X \to Y \mid x \mapsto F(x) \}$.
\end{itemize}

\section{Código Ejecutable: Python y Pseudocódigo}
La clase soporta bloques de código con \texttt{minted}:

\begin{itemize}[leftmargin=8mm]
    \item \textbf{\texttt{pythoncode}}: Bloque de código Python con syntax highlighting.
    \item \textbf{\texttt{pythonlib}}: Librería Python importada desde archivo externo.
    \item \textbf{\texttt{pythonexec}}: Código Python ejecutable que muestra stdout automáticamente.
    \item \textbf{\texttt{pseudocodigo}}: Pseudocódigo genérico en el lenguaje especificado.
\end{itemize}

\begin{warningbox}[Advertencia]
  Los entornos ejecutables requieren \texttt{-shell-escape} en LuaLaTeX y Python 3 instalado. Úsalos solo en borradores.
\end{warningbox}

\section{Listas de Propiedades y Condiciones}
\begin{itemize}[leftmargin=8mm]
    \item \textbf{\texttt{properties}}: Lista con viñetas y etiqueta \texttt{\string\propitem}.
    \item \textbf{\texttt{conditions}}: Sistema de ecuaciones condicionales con numeración automática vía \texttt{\string\conditem}.
\end{itemize}
...
\section{Convenciones y Supuestos Globales}
El entorno \texttt{convencionbox} marca supuestos que aplican a toda la sección:

\begin{convencionbox}[Notación]
  Todos los anillos considerados son conmutativos con identidad salvo mención expresa.
\end{convencionbox}

\section{Tournants Dangereux y Marcadores Marginales}
La clase incluye signos marginales al estilo EGA para marcar pasajes difíciles:

\begin{itemize}[leftmargin=8mm]
    \item \texttt{\string\tournantdangereux}: Curva de advertencia simple en el margen.
    \item \texttt{\string\doubletournantdangereux}: Curva doble para pasajes especialmente delicados.
    \item \texttt{\string\oldpage[EGA~I]{\{1.1.1\}}}: Marca marginal con referencia a obra clásica.
\end{itemize}

\section{Metadatos de Investigación}
Para documentar el estado de avance de un texto científico:

\begin{itemize}[leftmargin=8mm]
    \item \texttt{\string\dependencias\{Hilbert, Noether\}}: Lista de resultados previos requeridos.
    \item \texttt{\string\blueprint\{Localizar haz estructural\}}: Idea fuerza del capítulo.
\end{itemize}

\section{Referencias Clásicas (\texttt{\string\ega}, \texttt{\string\sga}, \texttt{\string\fac}, \texttt{\string\fga})}
Macros para citar las grandes obras del siglo XX directamente en el margen:

\begin{itemize}[leftmargin=8mm]
    \item \texttt{\string\ega\{I\}\{1.1.1\}}: Referencia a EGA I, página 1.1.1.
    \item \texttt{\string\sga\{3\}\{II.1.1\}}: Referencia a SGA 3, página II.1.1.
    \item \texttt{\string\fac\{1970\}}: Referencia a Foundations of Algebraic Geometry.
    \item \texttt{\string\fga\{1960\}}: Referencia a Fondements de la Géométrie Algébrique.
\end{itemize}
...
\begin{itemize}[leftmargin=8mm]
    ...
    \item \texttt{\string\TODO\{Completar\}}: \TODO{Completar demostración del lema 3}.
    \item \texttt{\string\DEBUG\{Signos\}}: \DEBUG{Verificar compatibilidad de signos}.
    \item \texttt{\string\NOTE\{Hartshorne\}}: \NOTE{Revisar versión de Hartshorne Cap. II}.
\end{itemize}
...
\begin{itemize}[leftmargin=8mm]
    \item \texttt{\string\alginput\{...\}}: Especifica la entrada del algoritmo.
    \item \texttt{\string\algoutput\{...\}}: Especifica la salida esperada.
    \item \texttt{\string\alginit\{...\}}: Describe la inicialización de variables.
    \item \texttt{\string\algparams\{...\}}: Lista parámetros formales.
    \item \texttt{\string\algstop\{...\}}: Define el criterio de parada.
    \item \texttt{\string\algstep\{N\}}: Etiqueta un paso numerado dentro de \texttt{algsteps}.
\end{itemize}
...
\section{Gráficos Metapost con \texttt{luamplib}}
La clase carga \texttt{luamplib} para incluir gráficos Metapost directamente en el documento:
...
\begin{figure}[htpb]
    ...
    \caption{Esfera de Riemann $\symbb{P}^1(\symbb{C})$.}
    \label{fig:esfera_riemann}
\end{figure}
\end{lstlisting}

La macro auxiliar \texttt{\string\incfig} busca automáticamente en \texttt{./figuras/} y \texttt{../figuras/}:

\begin{lstlisting}[language=TeX, basicstyle=\ttfamily\footnotesize, frame=single, backgroundcolor=\color{gray!5}]
\incfig[0.8]{diagrama-haces}
\end{lstlisting}

\section{Estilos TikZ Predefinidos}
La clase define estilos listos para geometría algebraica y programación no lineal:

\begin{itemize}[leftmargin=8mm]
    \item \texttt{feasible set}, \texttt{infeasible set}, \texttt{contour}: Conjuntos en optimización.
    \item \texttt{vector}, \texttt{tangent vector}, \texttt{descent vector}: Campos de direcciones.
    \item \texttt{point}, \texttt{neighborhood}, \texttt{hyperplane}: Elementos geométricos básicos.
    \item \texttt{pnldiagram}: Estilo maestro para diagramas de programación no lineal.
\end{itemize}
...
\section{Suite de Referencias Bourbaki / IHÉS}
Además de \texttt{cleveref}, la clase provee macros semánticas para citas cruzadas avanzadas:

\begin{itemize}[leftmargin=8mm]
    \item \texttt{\string\sref[autor]{label}}: Referencia con prefijo opcional \textbf{(autor, label)}.
    \item \texttt{\string\eref\{label\}}: Referencia inline simple \textbf{(label)}.
    \item \texttt{\string\csref\{label\}}: Nombre del entorno + número \textbf{Teorema (1.1)}.
    \item \texttt{\string\autonamedsref\{label\}}: Nombre automático + título del label.
    \item \texttt{\string\titref\{label\}\{Título personalizado\}}: Referencia con título customizado.
\end{itemize}
...
\begin{enumerate}[label=\bfseries\arabic*., leftmargin=10mm]
    ...
    \begin{lstlisting}[language=TeX, basicstyle=\ttfamily\scriptsize, frame=single, backgroundcolor=\color{gray!5}, breaklines=true, columns=fullflexible]
