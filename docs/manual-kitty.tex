...
\title[Emulador de Terminal Acelerado por GPU con Modus Vivendi Deuteranopia]{Guía Completa de Uso y Configuración de Kitty}
...
\begin{notabox}[Por qué Kitty y no Alacritty / WezTerm]
  Kitty combina aceleración GPU madura con soporte nativo de \texttt{extended\_keyboard} (esencial para Emacs PGTK), control remoto por socket y un ecosistema de scripts en Python extremadamente flexible.
\end{notabox}
...
\midrule
Fuente Principal & \texttt{Iosevka Term} (Light, 12.5pt) con altura de línea al 110\% \\
Negrita / Cursiva & Variantes Bold / Italic de Iosevka Term \\
Símbolos Matemáticos & Mapeo directo a \texttt{STIX Two Math} vía \texttt{symbol\_map} \\
\bottomrule
\end{tabularx}
\end{center}

\begin{warningbox}[Nota sobre Fuentes Matemáticas]
  Para que \texttt{STIX Two Math} se renderice correctamente en la terminal, la variante \texttt{Symbols Nerd Font Mono} debe estar instalada en el sistema. Si observas cuadrados en lugar de símbolos, revisa que la ruta de fuentes de Kitty apunte a la carpeta de \texttt{Nerd Fonts}.
\end{warningbox}
...
\begin{notabox}[Portapapeles en Wayland]
  La combinación \texttt{Ctrl+Shift+C/V} funciona de forma nativa con \texttt{wl-clipboard} en Wayland. Para copiar desde Emacs TUI a otras aplicaciones, asegúrate de tener \texttt{clipetty} o \texttt{osc52} activo en \texttt{kitty.conf}.
\end{notabox}
...
\section{Configuración de Ventanas y Decoraciones}
\begin{center}
\small
\begin{tabularx}{\linewidth}{l X}
\toprule
\textbf{Directiva} & \textbf{Valor y Efecto} \\
\midrule
\texttt{window\_padding\_width} & 2px de padding interno \\
\texttt{window\_border\_width} & 1px de borde de ventana \\
\texttt{active\_border\_color} & \#51afef (Azul zafiro) \\
\texttt{inactive\_border\_color} & \#1e1e1e (Fondo oscuro) \\
\texttt{hide\_window\_decorations} & \texttt{yes} (sin bordes de ventana del WM) \\
\texttt{cursor\_shape} & \texttt{block} (cursor bloque sólido) \\
\texttt{cursor\_blink\_interval} & 0 (sin parpadeo) \\
\bottomrule
\end{tabularx}
\end{center}
...
\toprule
\textbf{Rol} & \textbf{Color} \\
\midrule
Fondo & \#000000 \\
Texto Principal & \#ffffff \\
Cursor & \#f0c674 \\
Selección & \#3a3f58 / \#ffffff \\
Acento Primario & \#51afef (Azul) \\
Error / Crítico & \#ff6655 \\
Éxito / Info & \#7bc275 \\
\bottomrule
\end{tabularx}
\end{center}
