  ...
  (let ((root-dir (expand-file-name my/second-brain-path))
    ...
    (if (executable-find "bibtool")
        (progn
          ...
          (shell-command (format "find %s -name '*.bib' ! -name 'referencias.bib' -exec cat {} + | bibtool -s -d > %s" root-dir output-file))
          (message "📚 Bibliografía regenerada instantáneamente."))
      (message "🐢 Biblio: 'bibtool' no encontrado. Usando motor nativo (Lento)...")
      (let ((all-bib-files (directory-files-recursively root-dir "\\.bib$"))
            (seen-keys (make-hash-table :test 'equal))
            (count-removed 0))
        (with-temp-buffer
          (insert "% --- BIBLIOGRAFÍA GENERADA (MODO NATIVO) ---\n\n")
          (dolist (file all-bib-files)
            (unless (string-match-p "referencias.bib" (file-name-nondirectory file))
...
(defun my/brain-build-and-view ()
  ...
  (let* ((default-directory my/second-brain-path)
         ;; Se delega todo el trabajo pesado a latexmk
         (cmd "latexmk -pdflua -shell-escape -interaction=nonstopmode main.tex")
         (out-buf (get-buffer-create " *brain-compile* "))
         (proc (start-process-shell-command "brain-compile" out-buf cmd)))
    ...
    (with-temp-file dot-file
      ...
      (dolist (file files)
        (let ((node-name (file-name-base file))
          ...
          (while (string-match "\\\\\\(input\\|import\\){.*?\\([^/}]+\\)\\.tex}" content)
            (insert (format "  \"%s\" -> \"%s\";\n" node-name (match-string 2 content)))
            (setq content (replace-match "" nil nil content)))))
      (insert "}"))
    (shell-command (format "dot -Tpng %s -o %s" dot-file img-file))
    (if (file-exists-p img-file)
        (my/open-pdf img-file)
      (message "Error: Instala graphviz (sudo apt install graphviz)."))))
